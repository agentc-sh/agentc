...
"#;
    ...
    async fn echo(
        ...
        state.echo_count.fetch_add(1, Ordering::SeqCst);
    ...
    async fn observe_upload(State(state): State<Arc<TestState>>, body: Body) -> StatusCode {
        ...
        while let Some(chunk) = futures::StreamExt::next(&mut chunks).await {
            match chunk {
                Ok(chunk) => {
                    state.upload_bytes.fetch_add(chunk.len(), Ordering::SeqCst);
    ...
    async fn executor(builder: crate::client::builder::HttpClientBuilder) -> Executor<RustPython> {
        Executor::<RustPython>::builder("agentc_http_integration_test")
            .bundle(
                Bundle::single("agentc_http_integration_test", SOURCE).expect("bundle builds"),
            )
            .workers(2)
            .with_http(builder)
            .build()
    ...
    async fn call(executor: &Executor<RustPython>, export: &'static str, argument: String) -> Value {
        executor
            .execute(
    ...
    async fn requests_apply_defaults_and_preserve_original_objects() {
        ...
        assert_eq!(
            call(&executor, "send_identity", String::from("/relative")).await,
            json!(true),
        );
        assert_eq!(
    ...
    async fn response_values_and_body_lifecycle_cross_the_binding() {
        ...
        assert_eq!(
            decoded[2]
                ...
                .iter()
                .map(|value| value.as_str().expect("text chunk is a string"))
                .collect::<String>(),
            "a\r\n€\nb\rc",
        );
        assert_eq!(
            call(&executor, "latin1_text", format!("{base}/latin1")).await,
            json!("é"),
        );
        assert_eq!(
        ...
        for operation in ["read", "text", "json", "bytes", "text_chunks", "lines"] {
            assert_eq!(
                call(
                    &executor,
                    "limited",
                    format!("http://{address}/large|{operation}"),
                )
                .await,
                json!([8, true]),
            );
    ...
    async fn guest_timeout_cannot_extend_the_host_deadline() {
        ...
        let host_only = executor(
            HttpClient::builder().request_timeout(Duration::from_millis(10)),
        )
        .await;
        ...
        assert_eq!(
            call(&host_only, "deadline_case", format!("{url}|0.1")).await,
            json!("timeout"),
        );

        host_only.shutdown().await.expect("executor shuts down");
        ...
        assert_eq!(
            call(&guest_only, "deadline_case", format!("{url}|none")).await,
            json!("ok"),
        );

        guest_only.shutdown().await.expect("executor shuts down");

        let shorter_guest = executor(
            HttpClient::builder().request_timeout(Duration::from_millis(100)),
        )
        .await;
        ...
        shorter_guest.shutdown().await.expect("executor shuts down");
    }
    ...
    async fn streamed_upload_and_transport_errors_reach_python() {
        ...
        let unavailable = unused.local_addr().expect("unused address exists");
    ...
    async fn executor_cancellation_stops_a_blocked_upload() {
        ...
        if state.upload_bytes.load(Ordering::SeqCst) == 0 {
            state.first_upload_chunk.notified().await;
        }

        cancellation.cancel();

        assert!(
            in_flight.await.expect("caller joins").is_err(),
        );
        assert_eq!(state.upload_bytes.load(Ordering::SeqCst), 5);
