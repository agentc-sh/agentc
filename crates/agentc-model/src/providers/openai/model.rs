...
impl CompletionModel for OpenAiModel {
    ...
    async fn send(&self, request: CompletionRequest) -> Result<ChatCompletionStream, ModelError> {
        ...
        // top_k is not supported by OpenAI and is intentionally ignored here.

        // frequency_penalty: prefer the cross-provider field; fall back to OpenAI-specific params.
        if let Some(frequency_penalty) = request.frequency_penalty {
            additional["frequency_penalty"] = json!(frequency_penalty);
        }

        // presence_penalty: prefer the cross-provider field; fall back to OpenAI-specific params.
        if let Some(presence_penalty) = request.presence_penalty {
            additional["presence_penalty"] = json!(presence_penalty);
        }

        // seed: prefer the cross-provider field; fall back to OpenAI-specific params.
