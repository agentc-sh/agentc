...
use agentc_core::build::types::BuildEvent;

use crate::cli::ui::traits::{LogViewport, Spinner, StreamRenderer, Ui};

pub struct BuildStreamRenderer<U: Ui> {
    ui: U,
    verbose: bool,
    start: Instant,
...
impl<U: Ui> BuildStreamRenderer<U> {
...
impl<U: Ui> StreamRenderer<BuildEvent> for BuildStreamRenderer<U> {
    fn on_event(&mut self, event: &BuildEvent) {
        match event {
            BuildEvent::BuildStarted { agent_name } => {
            ...
            BuildEvent::TransformingAssets { count } => {
            ...
            BuildEvent::CompilerStdout(line) => {
                if let Some(vp) = &self.current_viewport {
                    vp.push(line);
                }
            }
            BuildEvent::CompilerStderr(line) => {
