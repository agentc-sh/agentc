...
This creates a `math/` directory with a `package.json`, a `tsconfig.json`, and a `src/index.ts` entry
point, with the tool development kit already configured. Install its dependencies:
