...
Read and write files from a component using the corresponding runtime library.
...
async def shout() -> None:
    workspace = await Directory.root().open_dir("/workspace")
    existing = await workspace.read_text("notes.md")

    await workspace.write_text("notes.out.md", existing.upper())
```
  </TabsContent>
</Tabs>
