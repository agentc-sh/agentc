...
try:
    async with Client().get("https://api.example.com/status") as response:
...
If the request violates the egress policy, the request raises an error whose message explains the
refusal.
...
- [agentc_http](/reference/runtime/http/python)
