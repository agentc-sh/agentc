...
When a TypeScript tool calls `input.emit?.(...)`, or a Python tool calls `input.emit(...)`, during
execution, the update is delivered to the client as a `CUSTOM` event. Note that AG-UI defines its
own `ACTIVITY_DELTA` event type; agentc tool emissions are not that. They arrive as `CUSTOM` events
because agentc's concept of an activity update does not map to AG-UI's `ACTIVITY_DELTA`.
