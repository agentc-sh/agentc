...
- [Runs](/reference/graph/react/http/runs): create runs and stream their events, and read run history.
...
Run creation streams its result as Server-Sent Events. Each event is a text frame with an `event`
line naming the event kind and a `data` line carrying the JSON payload. An empty line separates
consecutive events. The event kinds are documented under [Runs](/reference/graph/react/http/runs).
